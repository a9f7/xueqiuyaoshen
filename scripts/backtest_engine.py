...
把雪球大V（metalslime）的「观点」变成可验证的「事实」：
  ...
  - 观点久期：按发帖文本判定「短线 / 中长期 / 超长期」，匹配久期回测（短线 T+1/3/5，中长期 T+20/60/120，超长期 T+250/360/1000），T+5 总基线保留；
...
HORIZONS = [1, 3, 5, 7, 10, 20, 60, 120, 250, 360, 1000]  # 计算窗口（含超长期 250/360/1000 日）
PRIMARY_BENCH = "1.000300"  # 沪深300

# 观点久期：按发帖文本判定该观点是「短线 / 中长期 / 超长期」，再匹配对应验证窗口。
# 目的：短期观点不应拿 T+120 验证，中长期不该只盯 T+5，超长期（十年级）更不该用 T+20 判定。
...
LONG_KW = ["中长期", "中长线", "长线", "长持", "长拿", "价值投资", "价值股", "价值",
           ...
           "周线", "穿越周期"]
# 超长期：明确表达十年级/永久级持有的关键词，命中即判超长期（优先于中长期）。
SUPERLONG_KW = ["超长期", "超长线", "十年", "十年十倍", "十年持有", "十年不动", "拿十年",
                "永久持有", "永久", "留给下一代", "养老", "终身持有", "传家", "不动如山",
                "年线级别", "十年期", "长期持有不动", "陪跑十年"]
DURATIONS = {"短期": [1, 3, 5], "中长期": [20, 60, 120], "超长期": [250, 360, 1000]}
DURATION_LABEL = {"短期": "T+1/3/5（短窗口）", "中长期": "T+20/60/120（长窗口）",
                  "超长期": "T+250/360/1000（十年级）"}
DURATION_HUE = {"短期": "#00b894", "中长期": "#6c5ce7", "超长期": "#e17055"}
...
def classify_duration(text):
    """按发帖全文判定观点久期：短期 / 中长期 / 超长期 / 未明确（无明确表述）。

    逻辑：
      - 超长期关键词（十年级/永久持有）命中即判「超长期」，优先于中长期；
      - 其余按短/中长关键词命中数，同票取多者；
      - 都未命中则「未明确」（仍计入 T+5 总基线，不进匹配久期回测）。
    """
    sl = sum(1 for kw in SUPERLONG_KW if kw in text)
    if sl > 0:
        return "超长期"
