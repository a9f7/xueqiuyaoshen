...
HOLDINGS = {
    ...
    "000933.SZ": "神火股份",
    "000333.SZ": "美的集团",
    "0300.HK": "美的集团H",   # 2026-09-03 已由 A 股切换为 H 股
    "601579.SS": "会稽山",    # 2026-09-03 新进建仓（黄酒/食品饮料）
    "0005.HK": "汇丰控股",
    "2268.HK": "药明合联",
    "6990.HK": "科伦博泰",
    "0981.HK": "中芯国际",
...
FACTORS = {
    ...
    "XBI": ("美国生科", "ret"),
    "KWEB": ("中概互联网", "ret"),
    # --- 食品饮料 / 黄酒行业动量（面向新进的会稽山）---
    # 注：yfinance 不提供 A 股行业指数历史（399997 等仅返回当日 1 行），
    # 故用流动性好的同业个股作为行业动量代理。
    "600059.SS": ("黄酒同业(古越龙山)", "ret"),
    "600519.SS": ("白酒龙头(茅台)", "ret"),
}
...
PORTFOLIOS = {
    ...
    "组合·港股+A核心": {
        ...
        "会稽山": 67396,   # 2026-09-03 新进（黄酒）
        "美的集团H": 60480,  # 2026-09-03 由 A 股切换而来
    },
}
...
def main():
    ...
    if os.path.exists(cn_path):
        ...
        LAG_CN = ["CN_PMI", "CN_PPI同比", "CN_用电量同比", "CN_工业增加值同比",
                  ...
                  "CN_社零同比", "CN_CPI同比"]   # 统计类，次月发布
        # 注：CN_两融余额(沪) 为交易所实时披露的日频市场数据，不需滞后。
